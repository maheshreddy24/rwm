...
class RecurrentWorldModel(nn.Module):
    ...
    @torch.no_grad()
    def extract_representations(self, context, frame_times=None, state=None):
        # context: (B, N, 3, H, W)
        ...
        feats = self.encode(context.reshape(B * N, *img))
        ...
        for t in range(N):
            out, state = self._run_core(
                feats, frame_times, state, t, n_tok
            )
            memory.append(out)
