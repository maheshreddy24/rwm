"""Entry point: wires RVMDataset -> VideoSiamMAE -> EMA Trainer and runs training (JAX/Flax).

Same as `trainer.py`, except it trains against a masked-representation loss with an
EMA target encoder instead of the pixel-reconstruction loss (see optimisation/optimizer_ema.py).

Usage:
    python src_jax/trainer_ema.py --config configs/train_jax_ema.yaml [--resume]
"""
...
def main():
    ...
    parser.add_argument("--config", default="configs/train_jax_ema.yaml")
