...
def save_pytree_npz(path: str, tree: Any) -> None:
    ...
    # `.view` on load requires a C-contiguous buffer. `ascontiguousarray` documents
    # ndim >= 1 on its output, silently promoting 0-d leaves (scalars like `epoch`) to
    # shape (1,); the reshape undoes that so scalars round-trip as scalars.
    arrays = [
        np.ascontiguousarray(np.asarray(x)).reshape(np.shape(x)) for x in leaves
    ]
