...
class RVM(nn.Module):
    ...
    def forward(
        ...
        bs, Ts = source.shape[:2]
        Tt = target.shape[1]
        image = source.shape[2:]

        # we want to attend, all prev frames, rather than only relying on
        # source frames
        context = torch.cat([source, target], dim = 1)
        Tf = Ts + Tt
        with self._encoder_ctx():
            src = self._embed(context.reshape(bs * Tf, *image)) # bs, ts, ... -> bs * ts, ...
            # (B, 3, H, W) -> (B, 1 + N, D), 
            src = self._blocks(src) # output layer norm 
        src  = src.view(bs, Ts, *src.shape[1:])

        if state is None:
            state = src.new_zeros(bs, src.shape[2], self.d_enc)          # (B, 1+N, D)  s_0 = 0
        else:
            assert state.shape == (bs, src.shape[2], self.d_enc), (
                f"state {tuple(state.shape)} != {(bs, src.shape[2], self.d_enc)}"
            )
