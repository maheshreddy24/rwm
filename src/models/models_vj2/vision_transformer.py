...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None, training=False):
        ...
        print(f'[mahesh] inside forward')
        if not self.use_rope:
            pos_embed = self.interpolate_pos_encoding(x, self.pos_embed)
            print("[mahesh] interpolating pos emb")
        ...
        if self.check_temporal_dim(x.shape):
            ...
            print(f'[mahesh] bef embd {x.shape}')
            x = self.patch_embed(x)
            print(f'[mahesh] after patch embeding {x.shape}')
            mode = "video"
            if self.modality_embedding:
                x += self.video_mod_embed.repeat(x.shape[0], 1, 1)
                print('[mahesh] using video modality embedding')

        if not self.use_rope:
            x += pos_embed
            print("[mahesh] using rope embedding")

        if masks is not None:
            x = apply_masks(x, masks)
            masks = torch.cat(masks, dim=0)
            print(f'[mahesh] masks shape {masks.shape}')
        outs = []
        hier = []
        ...
        print(f'[mahesh] self.out_layers_distillation: {self.out_layers_distillation}')
